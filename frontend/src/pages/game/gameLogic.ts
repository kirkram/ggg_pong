	...
	*/
    ...
    function checkPaddleCollision(
      ...
      const isBallApproaching =
        (isLeft && ball.dx < 0 && ball.x >= x) || // ball moving left toward left paddle
        (!isLeft && ball.dx > 0 && ball.x <= x + w); // ball moving right toward right paddle

      if (
        isBallApproaching && // make sure it's actually approaching the paddle
        ball.x + ball.radius >= x &&
        ball.x - ball.radius <= x + w &&
        ball.y >= hitTop &&
        ball.y <= hitBottom &&
        ball.y < paddleCenterY // only allow collision before it passes the center
      ) {
        ball.dx *= -1;
        ball.dx *= speedUp;
        ball.dy *= speedUp;
