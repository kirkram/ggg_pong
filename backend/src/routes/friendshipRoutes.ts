...
export async function friendshipRoutes(app: FastifyInstance) {
  ...
  app.post('/friendships/request', { preHandler: [app.authenticate] }, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      return reply.status(500).send({ error: "Failed to set Friend status to Pending" });
    }
  });

  // Get pending friend requests for the current user
  app.get('/friendships/requests', { preHandler: [app.authenticate] }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.put('/friendships/decline', { preHandler: [app.authenticate] }, async (request: FastifyRequest, reply: FastifyReply) => {
    const currentUserId = request.user.id;
    ...
    if (receiver_id !== currentUserId) {
      return reply.status(403).send({ error: "You can only decline requests sent to you." });
    }
    ...
    try {
      ...
      return reply.send({ message: "Friend request declined and status updated." });
    } catch (error) {
  ...
  app.put('/friendships/accept', { preHandler: [app.authenticate] }, async (request: FastifyRequest, reply: FastifyReply) => {
    const currentUserId = request.user.id;
    const { sender_id, receiver_id } = request.body as {
      sender_id: number;
      receiver_id: number;
    ...
    // Ensure only the receiver can decline
    if (receiver_id !== currentUserId) {
      return reply.status(403).send({ error: "You can only accept requests sent to you." });
    }
    ...
    try {
      ...
      return reply.send({ message: "Friend request accepted and status updated." });
    } catch (error) {
      console.error("Error declining friend request:", error);
  ...
  app.put('/friendships/unfriend', { preHandler: [app.authenticate] }, async (request: FastifyRequest, reply: FastifyReply) => {
    const currentUserId = request.user.id;
    const { sender_id, receiver_id } = request.body as {
      sender_id: number;
      receiver_id: number;
